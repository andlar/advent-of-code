...
describe('day 3 tests', () => {
    ...
    describe('point generation', () => {
        it('should generate a wire of points to the right', () => {
            let points = genPoints(origin, 'R8', 'w1', {});
            expect(points['0:0']).toEqual('o');
            expect(points['3:0']).toEqual('w1-3');
            expect(points['8:0']).toEqual('w1-8');
        });

        it('should generate a wire of points to the right and up', () => {
            let points = genPoints(origin, 'R8,U2', 'w1', {});
            expect(points['0:0']).toEqual('o');
            expect(points['3:0']).toEqual('w1-3');
            expect(points['8:0']).toEqual('w1-8');
            expect(points['8:2']).toEqual('w1-10');
        });

        it('should generate a wire of points to the left and down', () => {
            let points = genPoints(origin, 'L8,D2', 'w2', {});
            expect(points['0:0']).toEqual('o');
            expect(points['-3:0']).toEqual('w2-3');
            expect(points['-8:0']).toEqual('w2-8');
            expect(points['-8:-2']).toEqual('w2-10');
        });

        it('should append when wires cross', () => {
            let points = genPoints(origin, 'R3,U3,L6,D5', 'w1', {});
            points = genPoints(origin, 'U5', 'w2', points);
            expect(points['0:3']).toEqual('w1-9&w2-3');
        });

        it('should not append when the wire crosses itself', () => {
            let points = genPoints(origin, 'R10,U2,L2,D4', 'w1', {});
            expect(points['8:0']).toEqual('w1-8');
        });
    });

    describe('finding distances', () => {
        it('should find intersections with other data "1"', () => {
            let points = genPoints(origin, wires[0], 'w1', {});
            points = genPoints(origin, wires[1], 'w2', points);
            expect(points['6:5']).toEqual('w1-15&w2-15');
            expect(points['3:3']).toEqual('w1-20&w2-20');
        });

        it('should find shortest distance with other data "1"', () => {
            let points = genPoints(origin, wires[0], 'w1', {});
            points = genPoints(origin, wires[1], 'w2', points);
            let distance = getClosestDistance(points);
            expect(distance).toEqual(6);
        });

        it('should find shortest distance with other data "2"', () => {
            let points = genPoints(origin, wires[2], 'w1', {});
            points = genPoints(origin, wires[3], 'w2', points);
            let distance = getClosestDistance(points);
            expect(distance).toEqual(159);
        });

        it('should find shortest distance with other data "3"', () => {
            let points = genPoints(origin, wires[4], 'w1', {});
            points = genPoints(origin, wires[5], 'w2', points);
            let distance = getClosestDistance(points);
            expect(distance).toEqual(135);
        });
    });

    it('should solve', () => {
        let points = genPoints(origin, wire1, 'w1', {});
        points = genPoints(origin, wire2, 'w2', points);
        let distance = getClosestDistance(points);
        expect(distance).toEqual(209);
    });
});

describe('day 3-b tests', () => {
    let origin;
    beforeEach(() => {
        origin = new Point(0, 0);
    });
